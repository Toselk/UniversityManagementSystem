...
void files_mg::save_std_data_into_files()
{
    ...
    for (auto &pair : Student::students) {
        ...
        for (auto &course : s.finished_courses) {
            stdFile << course << endl;
        }

        // Write the grade for finished courses to the file
        for (auto grade : s.gradesfor_finished_courses) {
            stdFile << grade << endl;
        }
        ...
        for (auto &courseinprogress : s.inprogress_courses) {
            stdFile << courseinprogress << endl;
        }
    }
    // Close the file stream
    stdFile.close();
...
void files_mg::load_std_data_from_files()
{
    ...
    for (int i = 0; i < counter; i++)
    {
        ...
        stdFile >> counterprereq;
        for (int j = 0; j < counterprereq; j++)
        {
            stdFile >> x;
            checkprereq.push_back(x);
        }
        ...
        Student student(name, id,email, password, max_hours_allowed, finished_courses, gradefor_finished_courses, inprogress_courses, academic_year , checkprereq);
