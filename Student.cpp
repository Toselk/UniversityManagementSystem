...
Student::Student(string name, int id,string email , string password, int max_hours_allowed, set<string>finished_courses, vector<double> gradesfor_finished_courses, set<string>inprogress_courses, string academic_year , vector<string> checkprereq) {
...
void Student::View_List_of_all_available_courses(Student s )
{
    ...
    {
        g.adjacency_list[it->second.name] = it->second.prerequisites;
    }
...
void Student::edit_data(Student s) {
    ...
    if (check_password == s.password) {
        ...
        string temppassword; 
        cin >> temppassword;
        Student::students[s.id].password = temppassword; 
        cout << "new password is " << temppassword << endl;
        cout << "done " << endl;
    }
    else {
...
void Student::get_cumulative_gpa(Student s) {
    ...
    vector<double>::iterator grade_it;
